...
if(isset($_POST['submit'])){
    ...
    if($result3){
        ...
        ;window.location.href='qiroah.php'</script>";
        
        }
    }
    
        $result = mysqli_query($conn, $sql);
        $sql2="SELECT * FROM ekstrakulikuler,dosen_pengajar WHERE id_ekstrakulikuler=5 AND ekstrakulikuler.dosen_NIP=dosen_pengajar.NIP";
        $result2=mysqli_fetch_array(mysqli_query($conn,$sql2));
        $status=$result2['status'];
?>
<h1 class="text-center mb-4 font-bold font-siz e-xl">Ekstrakulikuler Qiroah</h1>
<form action="qiroah.php" method="POST">
...
?>
            <tr>
                ...
                <td>
                    <a href="qiroah.php?nim_warga=<?= $rows['nim_warga'] ?>" class="btn btn-danger"><button>Hapus</button></a>
                    <?php $r=$rows['nim_warga']; 
                    if (isset($_GET['nim_warga']))  {
                        $aku=mysqli_query($conn,"DELETE FROM warga_ekstrakulikuler WHERE nim_warga='$r' and id_ekstrakulikuler=5"); 
                        if($aku){
                            echo "<script>alert('Data berhasil dihapus') ;window.location.href='qiroah.php'</script>";
                        }
                    }  ?>
                </td>
            </tr>
