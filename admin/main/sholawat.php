...
if(isset($_POST['submit'])){
    ...
    $sql3="UPDATE ekstrakulikuler,dosen_pengajar  SET jadwal_ekstrakulikuler='$jadwal',nama_dosen='$dosen',status='$status',materi='$materi' WHERE id_ekstrakulikuler=4 AND ekstrakulikuler.dosen_NIP=dosen_pengajar.NIP";
    $result3=mysqli_query($conn,$sql3);
    if($result3){
        echo "<script>alert('Data berhasil diubah')
        ;window.location.href='sholawat.php'</script>";
        
        }
    }
        $result = mysqli_query($conn, $sql);
        $sql2="SELECT * FROM ekstrakulikuler,dosen_pengajar WHERE id_ekstrakulikuler=4 AND ekstrakulikuler.dosen_NIP=dosen_pengajar.NIP";
        $result2=mysqli_fetch_array(mysqli_query($conn,$sql2));
        $status=$result2['status'];
?>
<h1 class="text-center mb-4 font-bold font-siz e-xl">Ekstrakulikuler Sholawat</h1>
<form action="sholawat.php" method="POST">
...
?>
            <tr>
                ...
                <td>
                    <a href="sholawat.php?nim_warga=<?= $rows['nim_warga'] ?>" class="btn btn-danger"><button>Hapus</button></a>
                    <?php $r=$rows['nim_warga']; 
                    if (isset($_GET['nim_warga']))  {
                        $aku=mysqli_query($conn,"DELETE FROM warga_ekstrakulikuler WHERE nim_warga='$r' and id_ekstrakulikuler=4"); 
                        if($aku){
                            echo "<script>alert('Data berhasil dihapus') ;window.location.href='sholawat.php'</script>";
                        }
                    }  ?>
                </td>
            </tr>
