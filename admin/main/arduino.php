...
if(isset($_POST['submit'])){
    ...
    echo $materi;
    $sql3="UPDATE ekstrakulikuler,dosen_pengajar  SET jadwal_ekstrakulikuler='$jadwal',nama_dosen='$dosen',status='$status',materi='$materi' WHERE id_ekstrakulikuler=3 AND ekstrakulikuler.dosen_NIP=dosen_pengajar.NIP";
    $result3=mysqli_query($conn,$sql3);
    if($result3){
        echo "<script>alert('Data berhasil diubah')
        ;window.location.href='arduino.php'</script>";
        
        }
    }
    
        $result = mysqli_query($conn, $sql);
        $sql2="SELECT * FROM ekstrakulikuler,dosen_pengajar WHERE id_ekstrakulikuler=3 AND ekstrakulikuler.dosen_NIP=dosen_pengajar.NIP";
        $result2=mysqli_fetch_array(mysqli_query($conn,$sql2));
        $status=$result2['status'];
?>
<h1 class="text-center mb-4 font-bold font-siz e-xl">Ekstrakulikuler Arduino</h1>
...
?>
            <tr>
                ...
                <td>
                    <a href="arduino.php?nim_warga=<?= $rows['nim_warga'] ?>" class="btn btn-danger"><button>Hapus</button></a>
                    <?php $r=$rows['nim_warga']; 
                    if (isset($_GET['nim_warga']))  {
                        $aku=mysqli_query($conn,"DELETE FROM warga_ekstrakulikuler WHERE nim_warga='$r' and id_ekstrakulikuler=3"); 
                        if($aku){
                            echo "<script>alert('Data berhasil dihapus') ;window.location.href='arduino.php'</script>";
                        }
                    }  ?>
                </td>
            </tr>
